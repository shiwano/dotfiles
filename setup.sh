...
main() {
	...
	find "${dotfiles_dir}/bin" -type f -maxdepth 1 -mindepth 1 |
		grep -v '.DS_Store' |
	...
	find "${dotfiles_dir}/dot.config" -maxdepth 1 -mindepth 1 |
		grep -v '.DS_Store' |
	...
	find "$dotfiles_dir" -maxdepth 1 -mindepth 1 -type d -name 'dot.*' |
		grep -v 'example' |
	...
	find "$dotfiles_dir" -maxdepth 1 -mindepth 1 -type f -name 'dot.*' |
		grep -v 'example' |
	...
	find "$dotfiles_dir" -maxdepth 1 -mindepth 1 -type f -name 'dot.*.example' |
		while IFS= read -r dotfile; do
			local dest
